...
export class OrdersService {
  ...
  async create(userId: string, dto: CreateOrderDto) {
    ...
    // Update product stock in batch (avoid sequential updates)
    ...
    try {
      ...
      if (user && user.email) {
        // Fetch all vendor details at once (avoid N+1)
        ...
        const vendors = await this.userModel
          ...
          .select('storeName firstName lastName email')
          .lean();
        ...
        const itemsWithVendorNames = items.map((item) => {
          let vendorName = 'Glovia Store';
          if (item.vendorId && vendorMap[item.vendorId.toString()]) {
            const vendor = vendorMap[item.vendorId.toString()];
            vendorName = vendor.storeName || `${vendor.firstName} ${vendor.lastName}`;
          }
          return {
            ...
            total: item.total,
            vendorName,
            vendorId: item.vendorId ? item.vendorId.toString() : undefined,
          };
        });
        ...
        await this.emailNotificationService.sendOrderConfirmedEmail(payload);

        // Prepare vendor information for super admin email
        const vendorOrderMap = new Map<string, { vendorName: string; vendorEmail: string; subtotal: number }>();
        itemsWithVendorNames.forEach(item => {
          if (item.vendorId) {
            ...
            const vendorInfo = vendorOrderMap.get(key);
            vendorInfo.subtotal += item.total;
          }
        });

        // Fetch vendor emails in batch (reuse vendorOrderMap if available)
        const vendorIdsForEmail = Array.from(vendorOrderMap.keys());
        if (vendorIdsForEmail.length > 0) {
          const vendorsForEmail = await this.userModel
            .find({ _id: { $in: vendorIdsForEmail } })
            .select('email')
            .lean();
          const vendorEmailMap = vendorsForEmail.reduce((acc, v) => {
            acc[v._id.toString()] = v.email;
            return acc;
          }, {} as Record<string, string>);
          vendorOrderMap.forEach((vendorInfo, vendorId) => {
            vendorInfo.vendorEmail = vendorEmailMap[vendorId] || '';
          });
        }

        // Send super admin email with all details
        const superAdminPayload = {
          ...
          vendors: Array.from(vendorOrderMap.values()),
        };
        await this.emailNotificationService.sendSuperAdminOrderEmail(superAdminPayload);
          ...
          try {
            ...
            if (vendor && vendor.email) {
              ...
              const vendorPayload = {
                ...
                // Vendor must not receive customer delivery address
                deliveryAddress: {
                  fullName: user.firstName + ' ' + user.lastName,
                  phone: user.phone || '',
                  province: '',
