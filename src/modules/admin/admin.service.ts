...
export class AdminService {
  ...
  async getDashboard() {
    ...
    // Execute all count and aggregation queries in parallel
    const [totalOrders, totalRevenue, totalCustomers, totalUsers, totalAdmins, totalVendors, totalProducts, pendingOrders] = await Promise.all([
      ...
      this.userModel.countDocuments({ role: UserRole.VENDOR }),
      this.productModel.countDocuments(),
      this.orderModel.countDocuments({ status: OrderStatus.PENDING })
    ]);

    // Execute dashboard data queries in parallel
    const [recentOrders, topProducts, revenueByMonth] = await Promise.all([
      this.orderModel
        .find()
        .sort({ createdAt: -1 })
        .limit(10)
        .select('_id userId orderNumber status total createdAt')
        .lean(),
      this.orderItemModel.aggregate([
        {
          $group: {
            _id: '$productId',
    ...
    // Fetch related users and products in parallel
    ...
    const [users, products] = await Promise.all([
      userIds.length > 0 ? this.userModel.find({ _id: { $in: userIds } }).select('firstName lastName email').lean() : Promise.resolve([]),
      productIds.length > 0 ? this.productModel.find({ _id: { $in: productIds } }).select('name price stockQuantity').lean() : Promise.resolve([])
    ]);
