...
export class CategoriesService {
  private readonly CACHE_KEY_ALL_CATEGORIES = 'categories:all';
  private readonly CACHE_TTL_CATEGORIES = 3600; // 1 hour
  ...
  async findAll() {
    // Check cache first
    const cachedCategories = await this.cacheService.get(this.CACHE_KEY_ALL_CATEGORIES);
    if (cachedCategories) {
    ...
    await this.cacheService.set(
      this.CACHE_KEY_ALL_CATEGORIES,
      result,
      this.CACHE_TTL_CATEGORIES,
    );
