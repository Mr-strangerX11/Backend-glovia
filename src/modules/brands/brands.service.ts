...
export class BrandsService {
  private readonly CACHE_KEY_ALL_BRANDS = 'brands:all';
  private readonly CACHE_TTL_BRANDS = 3600; // 1 hour
  ...
  async getAllBrands(includeProducts = false) {
    // Check cache first for non-extended requests
    if (!includeProducts) {
      const cachedBrands = await this.cacheService.get(this.CACHE_KEY_ALL_BRANDS);
      if (cachedBrands) {
        return cachedBrands;
      }
    }
    ...
    if (!includeProducts) {
      ...
      await this.cacheService.set(
        this.CACHE_KEY_ALL_BRANDS,
        brands,
        this.CACHE_TTL_BRANDS,
      );
      return brands;
    }
